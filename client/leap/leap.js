...
.run(function($rootScope, bopitSock, bopitAudio, leapController, $timeout, roundState) {
    ...
    $rootScope.$on("gesture:bop", function() {
        resetCanProceed();
        if (arePlaying) {
            $rootScope.$emit("bop");
    ...
    $rootScope.$on("gesture:twist", function() {
    ...
    $rootScope.$on("gesture:pull", function() {
        $rootScope.$emit("pull");
    ...
    leapController.loop(function(frame){
        if (frame.hands.length > 0 && canProceed) {
            ...
            if(hand.pitch() < -0.4) {
                if(normalFlag) $rootScope.$emit("gesture:twist");
                normalFlag = false;
            } else if(hand.stabilizedPalmPosition[0] > 100) {
                if(normalFlag) $rootScope.$emit("gesture:pull");
                normalFlag = false;
            } else if(Math.abs(hand.stabilizedPalmPosition[0]) < 30 && hand.stabilizedPalmPosition[2] < -5) {
                if (normalFlag) $rootScope.$emit("gesture:bop");
                normalFlag = false;
